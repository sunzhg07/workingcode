subroutine sm_basis
  ...
  integer:: bra,ket,bra_start,bra_end
  ...
  if(np==0)then 
    ...
    np_mbasis=1
    allocate(mbsp(1));mbsp=0
    allocate(mzp(1));mzp=0
    allocate(iparp(1));iparp=0
    allocate(mbsp_valid(1));mbsp_valid=1
    mbsp(1)=0
    mzp(1)=0
    iparp(1)=0
    mbsp_valid(1)=1
    ...
    allocate(mbsp(np_mbasis));mbsp=0
  ...
  if(iam==0)write(*,*)'number proton basis (no truncation)', np_mbasis

  tp_long=0
  low_long=0
  if(nn==0)then 
    write(*,*) 'number neutrons 0'
    nn_mbasis=1
    allocate(mbsn(1));mbsn=0
    ...
    allocate(mbsn(nn_mbasis)); mbsn=0
  ...
  if(iam==0)write(*,*)'number neutron states (no truncation): ', nn_mbasis
